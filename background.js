...
function checkAndLogUrl(url) {
  if (!url) {
    console.log("URL is undefined. This might be a special Chrome page or a page that the extension doesn't have permission to access.");
    return false;
  }
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (changeInfo.status === 'complete') {
    chrome.tabs.get(tabId, (currentTab) => {
      ...
      if (currentTab.url && (currentTab.url.includes("chatgpt.com") || currentTab.url.includes("claude.ai"))) {
...
function checkPermissions() {
  chrome.permissions.contains({
    ...
    if (result) {
      ...
      console.log("Please click the extension icon to grant the required permissions.");
    }
  });
}
